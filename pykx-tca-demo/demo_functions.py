...
def count_records_and_print(tab: kx.Table) -> kx.Table:
    """
    Print the number of records in a PyKX table & return the head of the table.
    """
    print(f"Num Records: {len(tab):,d}")
    return tab.head()


def plot_spreads_volatility_and_trade_volume(
    ax, sym: str, plot_num: int, spread_tab: kx.Table, volatility_dict: dict, trad_vol_tab: kx.Table
) -> None:
    ...
    ax[plot_num, 1].plot(
        trad_vol_tab["interval"], trad_vol_tab["volume"] / 1000000, color=volume_col
    )
    ...
    for k, v in d.items():
        ...
        if "," not in v:
            ...
            one_row = pd.DataFrame(
                {"total_memory": peak_mem_val, "memory_increment": increment_val}, index=[k]
            )
...
def parse_time_vals(d: dict) -> pd.DataFrame:
    ...
    for k, v in d.items():
        ...
        if "±" not in v:
            one_row = pd.DataFrame(
                {"avg_time": "", "avg_dev": "", "runs": "", "loops": ""}, index=[k]
            )
            ...
            one_row = pd.DataFrame(
                {"avg_time": avg_time, "avg_dev": avg_dev, "runs": runs, "loops": loops}, index=[k]
            )
...
def compare(df: pd.DataFrame, metric: str, syntax1: str, syntax2: str) -> float:
    factor = float(df[df["syntax"] == syntax1][metric].values) / float(
        df[df["syntax"] == syntax2][metric].values
    )
